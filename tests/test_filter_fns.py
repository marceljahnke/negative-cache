...
class FilterFnsTest(unittest.TestCase):
    def test_is_in_cache_filter_fn(self):

        data = {
            "1": torch.tensor(
                [[0, 0], [1, 1], [2, 2], [3, 3], [20, 20], [30, 30], [40, 40]]
            ),
            "2": torch.tensor(
                [[4, 4], [5, 5], [6, 6], [7, 7], [50, 50], [60, 60], [70, 70]]
            ),
        }
        age = torch.zeros(4)
        cache = negative_cache.NegativeCache(data, age)
        new_items = {
            "1": torch.tensor([[0, 0], [1, 1], [3, 3], [8, 8], [9, 9]]),
            "2": torch.tensor([[4, 4], [7, 7], [7, 7], [10, 10], [11, 11]]),
        }
        is_in_cache_filter_fn = filter_fns.IsInCacheFilterFn(keys=("1", "2"))
        mask = is_in_cache_filter_fn(cache, new_items)
        mask_expected = torch.tensor([False, True, False, True, True])
        self.assertTrue(torch.equal(mask_expected, mask))
    ...
    def test_is_in_cache_filter_fn_with_missing_keys(self):
        ...
        mask = is_in_cache_filter_fn(cache, new_items)
        mask_expected = torch.tensor([False, False, False, True, True])
        self.assertTrue(torch.equal(mask_expected, mask))
